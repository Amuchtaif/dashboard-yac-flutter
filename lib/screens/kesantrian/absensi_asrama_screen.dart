...
class _AbsensiAsramaScreenState extends State<AbsensiAsramaScreen> {
  ...
  Future<void> _fetchAsrama() async {
    ...
    try {
      final res = await _service.getDaftarAsrama();
      setState(() {
        _daftarAsrama = res;
        _isLoading = false;
      });
  ...
  Widget _buildAsramaCard(Asrama asrama) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(
            color: (isCompleted ? Colors.green : Colors.blue).withValues(alpha: 0.08),
            blurRadius: 15,
            offset: const Offset(0, 8),
          ),
        ],
            ...
            child: Row(
              children: [
                ...
                Container(
                  ...
                  height: 52,
                  decoration: BoxDecoration(
                    color: (isCompleted ? const Color(0xFF10B981) : const Color(0xFF3B82F6)).withValues(alpha: 0.1),
                    borderRadius: BorderRadius.circular(16),
                  ),
                  child: Icon(
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        asrama.nama.isNotEmpty ? asrama.nama : 'Kamar #${asrama.id}',
                        style: GoogleFonts.poppins(
                          fontWeight: FontWeight.bold,
                          fontSize: 16,
                          color: const Color(0xFF1E293B),
  ...
  void _handleAsramaClick(Asrama asrama) {
    if (asrama.sudahAbsen) {
      ...
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => DetailAsramaScreen(asrama: asrama),
        ),
      );
    } else {
      ...
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => InputAbsensiAsramaScreen(asrama: asrama),
        ),
      ).then((value) {
...
class DetailAsramaScreen extends StatefulWidget {
  final Asrama asrama;
  const DetailAsramaScreen({super.key, required this.asrama});
...
class _DetailAsramaScreenState extends State<DetailAsramaScreen> {
  ...
  Future<void> _fetchSantri() async {
    final list = await _service.getSantriByAsrama(widget.asrama.id);
    if (!mounted) return;
    setState(() {
      _santriList = list;
      _isLoading = false;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: const Color(0xFFF8FAFC),
      appBar: AppBar(
        title: Text(widget.asrama.nama, style: GoogleFonts.poppins(fontWeight: FontWeight.bold, fontSize: 16)),
        centerTitle: true,
          ...
          TextButton.icon(
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => InputAbsensiAsramaScreen(asrama: widget.asrama),
                ),
              );
            },
              ...
              return Container(
                ...
                child: Row(
                  children: [
                    CircleAvatar(
                      backgroundColor: const Color(0xFFF1F5F9),
                      child: Text(s.nama[0].toUpperCase(), style: GoogleFonts.poppins(fontWeight: FontWeight.bold)),
                    ),
                    const SizedBox(width: 16),
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(s.nama, style: GoogleFonts.poppins(fontWeight: FontWeight.bold, fontSize: 14)),
                          Text(s.kelas, style: GoogleFonts.poppins(fontSize: 12, color: Colors.grey)),
                        ],
                      ),
                    ),
  ...
  Widget _getStatusIcon(String status) {
    switch (status) {
      case 'Hadir':
        return const Icon(Icons.check_circle, color: Color(0xFF10B981), size: 24);
      case 'Izin':
        return const Icon(Icons.info_rounded, color: Color(0xFFF59E0B), size: 24);
      case 'Sakit':
        return const Icon(Icons.medical_services_rounded, color: Color(0xFF3B82F6), size: 24);
      default:
...
class InputAbsensiAsramaScreen extends StatefulWidget {
  final Asrama asrama;
  const InputAbsensiAsramaScreen({super.key, required this.asrama});

  @override
  State<InputAbsensiAsramaScreen> createState() => _InputAbsensiAsramaScreenState();
}
...
class _InputAbsensiAsramaScreenState extends State<InputAbsensiAsramaScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: Colors.white,
      appBar: AppBar(
        title: Text('Absensi ${widget.asrama.nama}', style: GoogleFonts.poppins(fontWeight: FontWeight.bold, fontSize: 16)),
        elevation: 0,
        backgroundColor: Colors.white,
        foregroundColor: Colors.black,
      ),
                      ...
                      return Container(
                        ...
                        child: Row(
                          children: [
                            ...
                            Row(
                              children: [
                                _buildStatusButton('H', s.status == 'Hadir', () {
                                  setState(() => s.status = 'Hadir');
                                }, const Color(0xFF10B981)),
                                const SizedBox(width: 8),
                                _buildStatusButton('I', s.status == 'Izin', () {
                                  setState(() => s.status = 'Izin');
                                }, const Color(0xFFF59E0B)),
                                const SizedBox(width: 8),
                                _buildStatusButton('S', s.status == 'Sakit', () {
                                  setState(() => s.status = 'Sakit');
                                }, const Color(0xFF3B82F6)),
                                const SizedBox(width: 8),
                                _buildStatusButton('A', s.status == 'Alfa', () {
  ...
  Widget _buildStatusButton(String label, bool isSelected, VoidCallback onTap, Color color) {
    return InkWell(
      onTap: onTap,
      child: Container(
        width: 40,
      ...
      child: SizedBox(
        ...
        child: ElevatedButton(
          ...
          child: _isSubmitting
              ...
              : Text('Simpan Absensi', style: GoogleFonts.poppins(fontWeight: FontWeight.bold, color: Colors.white, fontSize: 16)),
        ),
      ),
    );
  }
  ...
  Future<void> _submit() async {
    ...
    final result = await _service.submitAbsensi(widget.asrama.id, _santriList);
    if (!mounted) return;
